...
import { FileText, DollarSign, CheckCircle2, Clock, Users, Plus } from 'lucide-react'
...
export default function GrupoDashboardPage() {
  ...
  async function fetchData() {
    ...
    const [{ data: anesthesia }, { data: consultations }, { data: membrosData }] = await Promise.all([
      supabase.from('anesthesia_records').select('surgery_value, is_paid, has_glosa, procedure_date, patient_name, surgery_name').eq('group_id', grupoId).order('procedure_date', { ascending: false }),
      supabase.from('consultation_records').select('surgery_value, is_paid, has_glosa, consultation_date, patient_name, surgery_name').eq('group_id', grupoId).order('consultation_date', { ascending: false }),
            ...
            <div className="divide-y divide-slate-50">
              {membros.map((m: any) => (
                <div key={m.id} className="flex items-center gap-3 px-5 py-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-medium text-slate-900 truncate">{m.profiles?.full_name ?? '—'}</p>
