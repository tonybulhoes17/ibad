...
export default function GrupoFinanceiroPage() {
  ...
  const fetchAll = useCallback(async () => {
    ...
    const papelLocal = localStorage.getItem('grupo_papel') ?? ''
    const permissao = localStorage.getItem('grupo_permissao_consolidado') === 'true'
    setPapel(papelLocal)
    setPermissaoConsolidado(permissao)
  ...
  const podeVerFiltroMembro = papel === 'admin' || permissaoConsolidado
  ...
  const fichas = useMemo(() => {
    return allRecords.filter(r => {
      ...
      if (membroId && r.user_id !== membroId) return false
      return true
    })
  }, [allRecords, recordType, institutionId, planId, dateFrom, dateTo, membroId])
  ...
  const byMembro = useMemo(() => {
    if (!podeVerFiltroMembro) return []
    const map = new Map<string, { nome: string; count: number; value: number; paid: number }>()
    fichas.forEach(f => {
      const membro = membros.find(m => m.user_id === f.user_id)
      const nome = membro?.nome ?? 'Desconhecido'
      const cur = map.get(f.user_id) ?? { nome, count: 0, value: 0, paid: 0 }
      map.set(f.user_id, {
        nome,
        count: cur.count + 1,
        value: cur.value + (f.surgery_value ?? 0),
        paid: cur.paid + (f.is_paid ? (f.surgery_value ?? 0) : 0),
      })
    })
    return Array.from(map.values()).sort((a, b) => b.value - a.value)
  }, [fichas, membros, podeVerFiltroMembro])
  ...
  if (loading) return <div className="p-6 text-center text-slate-400">Carregando financeiro...</div>

  return (
    <div className="p-4 lg:p-6 max-w-7xl mx-auto">
      <div className="mb-5">
        <h1 className="text-xl font-bold text-slate-900">Dashboard Financeiro — Grupo</h1>
        <p className="text-sm text-slate-500">Controle consolidado de valores e pagamentos</p>
      </div>

      {/* Filtros */}
      <div className="card p-4 mb-5 grid grid-cols-2 lg:grid-cols-6 gap-3">
        <div>
          ...
          <select className="form-select text-sm" value={recordType} onChange={e => setRecordType(e.target.value as any)}>
            ...
            <option value="consultation">Pré-Anestésica</option>
          </select>
        </div>
        <div>
        ...
        {podeVerFiltroMembro && (
          <div>
            ...
            <select className="form-select text-sm" value={membroId} onChange={e => setMembroId(e.target.value)}>
              <option value="">Todos</option>
      ...
      {/* Por Anestesista — só admin e permissao_consolidado */}
      {podeVerFiltroMembro && byMembro.length > 0 && (
        <div className="card overflow-hidden">
          <div className="px-5 py-4 border-b border-slate-100">
            <h3 className="text-sm font-semibold text-slate-800">Por Anestesista</h3>
          </div>
          <table className="w-full text-sm">
            <thead className="bg-slate-50">
              <tr>
                {['Anestesista', 'Qtd', 'Total Previsto', 'Recebido', 'Taxa'].map(h => (
                  <th key={h} className="text-left px-5 py-2.5 text-xs font-semibold text-slate-500 uppercase tracking-wide">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-50">
              {byMembro.map(row => (
                <tr key={row.nome} className="hover:bg-slate-50">
                  <td className="px-5 py-3 font-medium text-slate-900">{row.nome}</td>
                  <td className="px-5 py-3 text-slate-500">{row.count}</td>
                  <td className="px-5 py-3 font-mono text-slate-700">{formatCurrency(row.value)}</td>
                  <td className="px-5 py-3 font-mono text-emerald-700">{formatCurrency(row.paid)}</td>
                  <td className="px-5 py-3">
                    <span className={`text-xs font-bold ${row.value ? (row.paid / row.value >= 0.8 ? 'text-emerald-600' : 'text-amber-600') : 'text-slate-400'}`}>
                      {row.value ? `${((row.paid / row.value) * 100).toFixed(0)}%` : '—'}
                    </span>
                  </td>
                </tr>
              ))}
