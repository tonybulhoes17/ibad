...
import {
  Search, Plus, Printer, Eye, Edit2, Trash2, Copy,
      ...
      {showFilters && (
        <div className="card p-4 mb-4 grid grid-cols-2 lg:grid-cols-4 gap-3 animate-fade-in">
          ...
          <div>
            ...
            <select className="form-select text-sm"
              onChange={e => setFilter('anesthesia_type', e.target.value)}>
              ...
              <tbody className="divide-y divide-slate-50">
                {fichas.map(ficha => (
                  <tr key={ficha.id} className="hover:bg-slate-50 transition-colors group">
                    ...
                    <td className="px-4 py-3 text-slate-500 text-xs">{ficha.institutions?.name ?? '—'}</td>
                    <td className="px-4 py-3">
                      <span className="badge-neutral">{ANESTHESIA_TYPES.find(t => t.value === ficha.anesthesia_type)?.label ?? ficha.anesthesia_type}</span>
                    </td>
            ...
            {fichas.map(ficha => (
              <div key={ficha.id} className="card p-4">
                <div className="flex items-start justify-between mb-2">
                  ...
                  {ficha.is_paid ? <span className="badge-success">Pago</span>
                    : ficha.has_glosa ? <span className="badge-danger">Glosa</span>
                    : <span className="badge-warning">Pendente</span>}
                </div>
