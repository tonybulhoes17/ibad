  ...
  return (
    <div className="p-4 lg:p-6 max-w-4xl mx-auto">
      ...
      <div className="card p-5 mb-4">
        ...
        {r.fasting && (
          <div className="mt-3">
            <p className="text-xs text-slate-400 mb-0.5">Jejum</p>
            <p className="font-medium text-slate-800 text-sm">{r.fasting}</p>
          </div>
        )}
        {r.medication_instructions && (
          <div className="mt-3">
            <p className="text-xs text-slate-400 mb-0.5">Medicamentos — Orientações</p>
            <p className="font-medium text-slate-800 text-sm">{r.medication_instructions}</p>
          </div>
        )}
        {r.observations && (
          <div className="mt-3">
            <p className="text-xs text-slate-400 mb-0.5">Observações</p>
            <p className="font-medium text-slate-800 text-sm">{r.observations}</p>
          </div>
        )}
      </div>
      ...
      <div className="card p-5">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 text-sm">
          <div>
            <p className="text-xs text-slate-400 mb-0.5">Plano</p>
            <p className="font-medium text-slate-800">{r.insurance_plans?.name ?? '—'}</p>
          </div>
          <div>
            <p className="text-xs text-slate-400 mb-0.5">Valor</p>
            <p className="font-medium text-slate-800 font-mono">
              {r.surgery_value ? `R$ ${Number(r.surgery_value).toFixed(2).replace('.', ',')}` : '—'}
            </p>
          </div>
          <div>
